...
//  Port of animated-gradient-background.tsx
//  No framer-motion, no CDN — pure vanilla JS
// ══════════════════════════════════════════════
(function initAnimatedGradient() {
  ...
  const gradientColors = [
    ...
    '#162828',   // dark teal
    '#1e3a3c',   // zichtbaar teal centrum
    '#162820',   // teal-donker
    '#1e1408',   // warm koper-donker
    '#120d07',   // donker warm rand
    '#0e1210'    // warm dark base
  ];
  ...
  function animate() {
    ...
    if (width <= startingGap - breathingRange) direction =  1;
    if (Breathing) width += direction * animationSpeed;

    const stops = gradientStops.map((s, i) => `${gradientColors[i]} ${s}%`).join(', ');
    el.style.background = `radial-gradient(${width}% ${width}% at 50% 20%, ${stops})`;
